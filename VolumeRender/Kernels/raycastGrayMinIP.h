...
std::string raycastGrayMinIP = R"(
/////////////////////////////////////////////////////////////////////////
// Front to Back MinIP rendering for Grayscale image              //
/////////////////////////////////////////////////////////////////////////
__kernel void renderMinIPGray_F2B(__global uint *d_output, 
	...
	{	
			...
			pos.z>myParam->clipPlaneCenter[4] && pos.z<myParam->clipPlaneCenter[5])
		{
			...
			if(bFirstPos)
			{
				...
				temp = read_imagef(transferFunc, transferFuncSampler, transfer_pos);
			...
			if( sample.w>=myParam->chR[0] && sample.w<=myParam->chR[1] )
			{
				...
				temp.x = fmin(temp.x, color.x);
				temp.y = fmin(temp.y, color.y);
				temp.z = fmin(temp.z, color.z);
				transp = fmax(transp, 1.0f - color.w);
			}
