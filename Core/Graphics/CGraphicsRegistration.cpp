...
CGraphicsRegistration::CGraphicsRegistration()
{
    ...
    registerItem(std::make_shared<CProxyGraphicsComplexPolyFactory>());
    registerItem(std::make_shared<CProxyGraphicsEllipseFactory>());
    registerItem(std::make_shared<CProxyGraphicsPointFactory>());
    registerItem(std::make_shared<CProxyGraphicsPolygonFactory>());
    registerItem(std::make_shared<CProxyGraphicsPolylineFactory>());
    registerItem(std::make_shared<CProxyGraphicsRectFactory>());
    registerItem(std::make_shared<CProxyGraphicsTextFactory>());
}

const CProxyGraphicsAbstractFactory &CGraphicsRegistration::getProxyFactory() const
{
    return m_proxyFactory;
}

const CGraphicsAbstractFactory &CGraphicsRegistration::getQtBasedFactory() const
{
    return m_factory;
}
...
void CGraphicsRegistration::registerItem(const std::shared_ptr<CProxyGraphicsFactory> &pFactory)
{
    assert(pFactory);
    m_proxyFactory.getList().push_back(pFactory);
    //Passage par lambda -> pFactory par valeur pour assurer la portée du pointeur
    auto pCreatorFunc = [pFactory]{ return pFactory->create(); };
    m_proxyFactory.registerCreator(pFactory->getType(), pCreatorFunc);
}
